...
const Navbar = () => {
    ...
    return (
        <div className='flex items-center justify-between text-sm py-4 mb-5 border-b border-b-gray-400'>
            <NavLink to="/"><img className='w-44 cursor-pointer h-20' src={assets.logo} alt="Logo" /></NavLink>
            <ul className='hidden md:flex items-start gap-5 font-medium'>
                <NavLink to="/">
                    <li className='py-1'>HOME</li>
                    <hr className='border-none outline-noneh-0.5 bg-primary w-3/5 m-auto hidden' />
                </NavLink>
                <NavLink to="/doctors">
            ...
            <div className='flex items-center gap-4'>
                {
                    token && userData
                     ? <div className='flex items-center gap-2 cursor-pointer group relative'>
                        ...
                        <div className='absolute top-0 right-0 text-base text-gray-600 z-20 hidden group-hover:block'>
                ...
                <div className={`${showMenu ? 'fixed w-full' : 'h-0 w-0'} md:hidden right-0 top-0 bottom-0 z-20 overflow-hidden bg-white transition-all}`}>
                    <div className="flex items-center justify-between px-5 py-6 ">
                        <img src={assets.logo} alt="Close Icon" className="w-36 md:hidden cursor-pointer" onClick={() => setShowMenu(false)} />
                        <img src={assets.cross_icon} alt="Menu Icon" onClick={()=>setShowMenu(false)}  className='w-7'/>
                    </div>
